...
3. No **Cloudflare Dashboard → Pages → apoio → Settings → Environment variables**,
...
```bash
wrangler pages project create apoio
```
...
**Cloudflare Dashboard → Pages → apoio → Settings → Functions → D1 database bindings**
...
```bash
wrangler pages deploy . --project-name=apoio
```

Ao final, o Wrangler mostra a URL pública (algo como `https://apoio.pages.dev`).
...
Em **Pages → apoio → Custom domains**, adicione, por exemplo,
...
A Administração de usuários agora usa paginação no servidor, busca e filtros por unidade, função e status. O administrador escolhe 10, 20, 50 ou 100 registros por página.

## 2.9.1
O Apoio mantém o schema do portal-saude-db, inclusive comunicação interna, suporte, chamados e configuração de retenção compartilhados com o eMulti.

### Chat v2.9.1
Presença online, leitura/não lidas, suporte encerrável e badges no menu usam o `portal-saude-db`. Execute `database/update.sql` no D1 do Portal antes do deploy desta versão.
